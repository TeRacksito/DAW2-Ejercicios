...
class QR
{
    ...
    function __construct()
    {
        ...
        $json = file_get_contents('version_information.json');
        $data = json_decode($json, true);
        ...
        $json = file_get_contents('format_information.json');
        $data = json_decode($json, true);
    ...
    public function generate(string $data, string $ecc_level, string $filename, int $scale = 10): array
    {
        ...
        list($mask, $penalty, $masked_map) = $this->determineMask($map, $shadow_map);
    ...
    public function determineVersion(string $data): int
    {
        ...
        foreach ($this->versions as $version => $ecc_levels) {
            ...
            if (($length > 9 ? $length + 1 : $length) <= $max_codewords) {
                return $version;
            }
        }
    }
...
init_tables();
$qr = new QR();
$qr->generate('A que está chulo el QR hecho con PHP desde 0. La verdad es que toca los huevos que no veas porque tienes que tener en cuenta un montón de mierdas y es algo complejo pero bueno. He conseguido soportar de la version 1 a la 40 (todas las normales). También soporto los cuatro modos de corrección de error. Así como las 8 mascaras, que se evalúan y se aplica la mejor automáticamente. Lo que no soporto son los diferentes modos, pues solo hago uso del modo binario (Existen otros, que podrían ser más eficientes para texto y cosas así). Tampoco hago optimizaciones de segmentos, que es un tema complejo para optimizar aún más el espacio que ocupa el texto cuando trabajas en modo alfanumérico. Lo guapo es que el script, automáticamente, elige la versión (1-40, determina el tamaño del QR) que mejor es para el texto de entrada.', 'Q', 'qr.png');
